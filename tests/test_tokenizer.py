...
def test_encode_iterable_memory_usage_macos():
    """
    macOS-compatible version of test_encode_iterable_memory_usage.
    Instead of using rlimit, we monitor memory usage with psutil.
    """
    tokenizer = get_tokenizer_from_vocab_merges_path(
        vocab_path=VOCAB_PATH,
        merges_path=MERGES_PATH,
    ...
    with open(FIXTURES_PATH / "tinystories_sample_5M.txt") as f:
        ...
        for _id in tokenizer.encode_iterable(f):
            ...
            # Check if memory increase exceeds limit
            if current_memory - initial_memory > max_allowed_increase:
                pytest.fail(f"Memory usage exceeded limit: {(current_memory - initial_memory) / 1e6:.2f}MB > 1MB")
    
    print(f"Peak memory increase: {(peak_memory - initial_memory) / 1e6:.2f}MB")


@pytest.mark.xfail(reason="Tokenizer.encode is expected to take more memory than allotted (1MB).")
def test_encode_memory_usage_macos():
    """
    macOS-compatible version of test_encode_memory_usage.
    We expect this test to fail, since Tokenizer.encode is not expected to be memory efficient.
    """
    tokenizer = get_tokenizer_from_vocab_merges_path(
        vocab_path=VOCAB_PATH,
    ...
    with open(FIXTURES_PATH / "tinystories_sample_5M.txt") as f:
        ...
        before_encode = process.memory_info().rss
        
        # Encode the entire content at once
        _ = tokenizer.encode(contents)
        
        # Measure memory after encoding
        after_encode = process.memory_info().rss
        memory_increase = after_encode - before_encode
        
        print(f"Memory increase during encode: {memory_increase / 1e6:.2f}MB")
        
        # Check if memory increase exceeds limit
        if memory_increase > max_allowed_increase:
            pytest.fail(f"Memory usage exceeded limit: {memory_increase / 1e6:.2f}MB > 1MB")
